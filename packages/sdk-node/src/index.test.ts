...
describe('RollgateClient', () => {
  ...
  describe('init', () => {
    it('should fetch flags on init', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ flags: { 'test-flag': true } }));
  ...
  describe('events', () => {
    ...
    it('should emit flags-updated on every fetch', async () => {
      mockFetch.mockResolvedValue(createMockResponse({ flags: { flag: true } }));
  ...
  describe('SSE streaming', () => {
    ...
    it('should update flags when SSE message received', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ flags: { 'initial-flag': false } }));
      ...
      emitSSEEvent('init', { flags: { 'initial-flag': true, 'new-flag': true } });

      expect(client.isEnabled('initial-flag')).toBe(true);
      expect(client.isEnabled('new-flag')).toBe(true);
      expect(updateHandler).toHaveBeenCalledWith({ 'initial-flag': true, 'new-flag': true });
    });
