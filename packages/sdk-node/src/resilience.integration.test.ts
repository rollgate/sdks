 ...
 * working together: retry, circuit breaker, timeout, cache, deduplication.
 */
...
describe('Resilience Integration Tests', () => {
  ...
  describe('Combined Resilience Scenario', () => {
    beforeEach(() => {
      jest.useFakeTimers();
    });
    ...
    it('should handle degraded service with retry, circuit breaker, and cache', async () => {
      jest.useRealTimers(); // Use real timers for reliability

      const cache = new FlagCache({ ttl: 100, staleTtl: 500 }); // Short TTLs for testing
      const cb = new CircuitBreaker({ failureThreshold: 3, recoveryTimeout: 50 });
      const dedup = new RequestDeduplicator();
      ...
      const fetchFlags = async (): Promise<Record<string, boolean>> => {
        return dedup.dedupe('flags', async () => {
          // Check cache first
          ...
          if (!cb.isAllowingRequests()) {
            if (cached) return cached.flags; // Use stale cache
          ...
          return cb.execute(async () => {
            const result = await fetchWithRetry(
              async () => {
                ...
                return { feature_a: true, feature_b: false };
              },
              { maxRetries: 1, baseDelayMs: 1, jitterFactor: 0 } // Minimal delays
            );

            if (!result.success) throw result.error;

            // Update cache
            cache.set('flags', result.data!);
            return result.data!;
          });
        });
      };

      // Scenario 1: Server available - should fetch and cache
      const flags1 = await fetchFlags();
      expect(flags1).toEqual({ feature_a: true, feature_b: false });
      expect(serverCallCount).toBe(1);

      // Scenario 2: Server unavailable - should use cache
      serverAvailable = false;
      const flags2 = await fetchFlags();
      expect(flags2).toEqual({ feature_a: true, feature_b: false }); // From cache
      expect(serverCallCount).toBe(1); // No new calls - cache hit

      // Scenario 3: Make cache stale and trigger circuit breaker failures
      await new Promise((resolve) => setTimeout(resolve, 150)); // Wait for cache to become stale
      ...
      // Scenario 4: Server recovers
      serverAvailable = true;
      await new Promise((resolve) => setTimeout(resolve, 60)); // Wait for recovery timeout

      // Should use stale cache or fetch fresh
      const flags4 = await fetchFlags();
      expect(flags4).toEqual({ feature_a: true, feature_b: false });
      // Circuit may be in half-open or closed depending on success threshold
