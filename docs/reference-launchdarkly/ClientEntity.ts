...
function makeSdkConfig(options: SDKConfigParams, tag: string) {
  ...
  const maybeTime = (seconds?: number) =>
    isSet(seconds) ? seconds / 1000 : undefined;
  ...
  if (options.streaming) {
    ...
    cf.streaming = true;
    cf.streamInitialReconnectDelay = maybeTime(
      options.streaming.initialRetryDelayMs,
    );
  }
  ...
  if (options.hooks) {
    cf.hooks = options.hooks.hooks.map(
      (hook) =>
        new TestHook(hook.name, hook.callbackUri, hook.data, hook.errors),
    );
  }
    ...
    switch (params.command) {
      ...
      case CommandType.IdentifyEvent: {
        ...
        await this._client.identify(
          identifyParams.user || identifyParams.context,
        );
        return undefined;
      }
...
export async function newSdkClientEntity(options: CreateInstanceParams) {
  ...
  logger.info(
    `Creating client with configuration: ${JSON.stringify(options.configuration)}`,
  );
